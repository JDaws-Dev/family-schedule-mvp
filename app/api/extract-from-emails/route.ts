...
export async function POST(request: NextRequest) {
  try {
    const { emailIds, familyId } = await request.json();

    if (!emailIds || !Array.isArray(emailIds) || emailIds.length === 0 || !familyId) {
      return NextResponse.json({ error: "Missing emailIds or familyId" }, { status: 400 });
    }

    console.log(`[extract-from-emails] Extracting events from ${emailIds.length} email(s)`);
    ...
    for (const emailId of emailIds) {
      // Try each Gmail account until we find the email
      for (const account of gmailAccounts) {
        try {
          console.log(`[extract-from-emails] Processing email ${emailId} from ${account.gmailEmail}`);
...
IMPORTANT: Extract ALL events, schedules, and dates mentioned. If it's a full season schedule, extract every game/practice date.`
          ...
          if (result.eventsFound > 0 && result.events) {
            console.log(`[extract-from-emails] Extracted ${result.eventsFound} event(s) from: ${subject}`);
          ...
          // Found the email, break out of account loop
          break;
        } catch (error: any) {
          // If error is 404, try next account. Otherwise log and continue
          if (error.code === 404) {
            continue;
          }
          console.error(`[extract-from-emails] Error processing email ${emailId}:`, error);
        }
      }
    }

    console.log(`[extract-from-emails] Total events extracted: ${allEvents.length}`);
    ...
    console.error("[extract-from-emails] Error:", error);
    return NextResponse.json(
      {
        error: "Failed to extract events from emails",
